...
// Profile Image component
const ProfileImage = memo(({ businessCardData, cleanImageUrl }) => {
  const [imgSrc, setImgSrc] = useState('');
  const [localLoading, setLocalLoading] = useState(true);
  const [retryCount, setRetryCount] = useState(0);
  const maxRetries = 2;
  ...
  useEffect(() => {
    ...
    const photoUrl = businessCardData?.photo_url;
    ...
    if (!photoUrl) {
      ...
      const formattedUrl = BusinessCardService.formatPhotoUrl(cleanImageUrl(photoUrl));
      setImgSrc(formattedUrl);
    }
  }, [businessCardData, cleanImageUrl]);
  ...
  const handleImageError = (e) => {
    if (retryCount < maxRetries) {
      ...
      if (photoUrl) {
        setTimeout(() => {
          // Add a cache-busting parameter to force reload
          const newUrl = BusinessCardService.formatPhotoUrl(cleanImageUrl(photoUrl)) + `?retry=${retryCount}`;
          setImgSrc(newUrl);
          setRetryCount(prevCount => prevCount + 1);
        }, 500);
      } else {
  ...
  return (
    <div className="image-wrapper">
      ...
      <img 
        src={imgSrc} 
        alt={`${businessCardData?.display_name || 'User'}'s profile`}
        onLoad={() => setLocalLoading(false)}
        onError={handleImageError}
        style={{ opacity: localLoading ? 0.5 : 1 }}
      />
...
// Company Logo component
const CompanyLogo = memo(({ businessCardData, cleanImageUrl }) => {
  const [imgSrc, setImgSrc] = useState('');
  const [localLoading, setLocalLoading] = useState(true);
  const [retryCount, setRetryCount] = useState(0);
  const maxRetries = 2;
  ...
  useEffect(() => {
    ...
    const logoUrl = businessCardData?.company_logo_url;
    if (!logoUrl) {
      setLocalLoading(false);
      setImgSrc('');
      return;
    ...
    setImgSrc(BusinessCardService.formatPhotoUrl(cleanImageUrl(logoUrl)));
  }, [businessCardData, cleanImageUrl]);

  const handleImageError = (e) => {
    if (retryCount < maxRetries) {
      setTimeout(() => {
        setImgSrc(BusinessCardService.formatPhotoUrl(cleanImageUrl(imgSrc)));
        setRetryCount(prevCount => prevCount + 1);
      }, 500);
    } else {
      setLocalLoading(false);
...
const SlugProfile = () => {
  ...
  // Function to generate vCard content
  const generateVCard = useCallback((businessCard, user) => {
    // Start building the vCard content
    let vCardContent = 'BEGIN:VCARD\n';
    vCardContent += 'VERSION:3.0\n';
    
    // Add name
    vCardContent += `FN:${businessCard?.display_name || user?.full_name}\n`;
    
    // Format name components when available
    if (user?.first_name && user?.last_name) {
      vCardContent += `N:${user.last_name};${user.first_name};;;\n`;
    }
    
    // Add organization if company name exists (extracted from title or explicit company field)
    if (businessCard?.company_name) {
      vCardContent += `ORG:${businessCard.company_name}\n`;
    }
    
    // Add job title if exists
    if (businessCard?.title) {
      vCardContent += `TITLE:${businessCard.title}\n`;
    }
    
    // Add phone if exists - support multiple phone types
    if (businessCard?.contact?.phone) {
      vCardContent += `TEL;TYPE=CELL:${businessCard.contact.phone}\n`;
    }
    
    // Add email if exists
    if (businessCard?.email) {
      vCardContent += `EMAIL;TYPE=WORK:${businessCard.email}\n`;
    }
    
    // Add website if exists
    if (businessCard?.website) {
      const website = businessCard.website.startsWith('http') ? 
    ...
    // Add Kinvo URL
    const baseUrl = process.env.NODE_ENV === 'development' ? 'http://localhost:5173/' : 'https://kinvo.com/';
    vCardContent += `URL;TYPE=PROFILE:${baseUrl}${businessCard.slug}\n`;
    
    // Add note with bio if exists
    if (businessCard?.bio) {
      // Escape special characters in the note field
      const escapedBio = businessCard.bio
        .replace(/,/g, '\\,')
        .replace(/;/g, '\\;')
        .replace(/\n/g, '\\n');
      vCardContent += `NOTE:${escapedBio}\n`;
  ...
  const saveContact = useCallback((e) => {
    ...
    const vCardContent = generateVCard(businessCardData, userData);
    const blob = new Blob([vCardContent], { type: 'text/vcard' });
    const url = URL.createObjectURL(blob);
    
    const filename = `${businessCardData?.display_name || userData?.full_name || 'contact'}.vcf`;
  ...
  }, [businessCardData, userData, generateVCard]);
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 404) {
            ...
            throw new Error(`Error fetching business card: ${response.status}`);
          }
        }
        
        const data = await response.json();
        setBusinessCardData(data.business_card);
        ...
        if (error.message === 'Business card not found') {
          setError('Business card not found. This page may have been moved or doesn\'t exist.');
        } else {
          setError('Something went wrong while loading this business card. Please try again later.');
        }
        setLoading(false);
      }
    };
      ...
      <div className="profile-card">
        ...
        <div className="company-logo-container">
          <CompanyLogo 
            ...
            cleanImageUrl={cleanImageUrl} 
          />
          {!businessCardData?.company_logo_url && (
            <div className="no-logo-placeholder">No company logo</div>
          )}
        </div>
        ...
        <div className="profile-content">
          ...
          <h1>{businessCardData?.display_name || userData?.full_name}</h1>
          {businessCardData?.title && <h2>{businessCardData.title}</h2>}
          
          {businessCardData?.bio && (
            <p className="bio-text">{businessCardData.bio}</p>
          )}
          ...
          <div className="contact-buttons">
            ...
            {businessCardData?.contact?.phone && (
              <div className="contact-button">
                <a href={`tel:${businessCardData.contact.phone}`}>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
                  </svg>
                  Call
            ...
            {businessCardData?.website && (
              <div className="contact-button">
                <a 
                  href={businessCardData.website.startsWith('http') ? 
                    businessCardData.website : 
                    `https://${businessCardData.website}`} 
                  target="_blank" 
                  rel="noopener noreferrer"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
